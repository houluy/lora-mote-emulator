...
appkey, nwkkey = [bytes.fromhex(device_conf.get('keys').get(key)) for key in ['AppKey', 'NwkKey']]
...
try:
    ...
    mote = mac.Mote(joineui, deveui, appkey, nwkkey, device_file, version=args.version)
